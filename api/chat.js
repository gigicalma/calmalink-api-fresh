...
index 82cbd095b261c818a782cafcde523e636741a60a..9d69f25906420735226e03ffa4f8bddb9cebbb8a 100644
--- a/api/chat.js
+++ b/api/chat.js
@@ -1,249 +1,266 @@
 // api/chat.js
...
+// Restrict cross-origin requests to live domains
...
+// Meditation categories available
...
+// Uploaded audio files hosted publicly
...
+// Quick-start recognizer: checks last user message for a request to start calm_breath
...
+      try {
+        language = JSON.parse(tc.arguments)?.language === "es" ? "es" : "en";
+      } catch {}
...
+      const text =
+        out.output_text ||
+        (language === "es" ? "Aquí tienes tu práctica." : "Here is your practice.");
+      return ok(res, {
+        message: text,
+        tool: {
+          name: "get_meditation",
...
+    // 3) If no tool call, return the model's text directly
     const normalText =
